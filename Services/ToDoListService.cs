...
public class ToDoListService : IToDoListService
{
    ...
    private void AddDeadline(ToDoItem newTask, NewItemDTO task)
    {
    ...
    private void AddPriority(ToDoItem newTask, NewItemDTO task)
    {
        string pattern = @"!\d";
    ...
    private void EditDeadline(ToDoItem newTask, EditTaskDTO task)
    {
        var regex = new Regex(@"!before\s*(\d{2}[.-]\d{2}[.-]\d{4})", RegexOptions.IgnoreCase);
        var match = regex.Match(task.Name);
    ...
    private void EditPriority(ToDoItem newTask, EditTaskDTO task)
    {
        string pattern = @"!\d";
